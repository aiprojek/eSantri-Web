        ...
        <div className="space-y-4 h-full flex flex-col">
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 shrink-0 overflow-x-auto">
                <nav className="flex -mb-px">
                    ...
                    <button onClick={() => setActiveTab('gudang')} className={`flex-1 py-3 px-4 border-b-2 font-bold text-sm whitespace-nowrap ${activeTab === 'gudang' ? 'border-teal-600 text-teal-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}><i className="bi bi-houses mr-2"></i> Gudang</button>
                    <button onClick={() => setActiveTab('supplier')} className={`flex-1 py-3 px-4 border-b-2 font-bold text-sm whitespace-nowrap ${activeTab === 'supplier' ? 'border-teal-600 text-teal-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}><i className="bi bi-truck mr-2"></i> Vendor / Supplier</button>
