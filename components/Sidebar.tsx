  ...
  return (
    ...
    <aside className={`fixed top-0 left-0 z-40 w-64 h-screen transition-transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 bg-teal-800 text-white no-print flex flex-col`}>
        ...
        <div className="px-3 mb-2">
            <div className="p-3 bg-teal-900/50 rounded-lg border border-teal-700">
                ...
                <div className="flex items-center gap-1 mt-0.5">
                    ...
                    {canManageSync && currentUser?.role !== 'admin' && <p className="text-[10px] text-yellow-300 border border-yellow-600 px-1 rounded" title="Wakil Admin (Sync)">Wakil</p>}
                </div>
            </div>
        </div>
      ...
      <div className="p-4 border-t border-teal-700 space-y-2">
          {settings.multiUserMode && <button onClick={handleLogout} className="flex items-center justify-center w-full p-2 bg-red-800 hover:bg-red-700 rounded-lg text-sm transition-colors text-white mb-2"><i className="bi bi-box-arrow-left mr-2"></i><span>Keluar</span></button>}
          <button onClick={handleSyncClick} disabled={isSyncing || syncStatus === 'syncing'} className={`flex items-center justify-center w-full p-2 rounded-lg text-sm transition-colors group ${getButtonClass()}`} title="Upload/Download data Cloud">{renderSyncStatus()}</button>
      </div>
    </aside>
