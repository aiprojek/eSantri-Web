...
export const generateRaporFormHtml = (
    ...
    targetSantri.forEach((s, index) => {
        ...
        const rowCells = interactiveCells.map(col => {
            ...
            if (col.type === 'input') {
                return `<td class="p-1 border border-black"><input type="text" id="${fieldId}" name="${fieldId}" oninput="calculateRow(${s.id})" class="w-full h-full p-1.5 text-center bg-white focus:bg-blue-50 outline-none transition-colors rounded text-sm font-medium focus:ring-2 focus:ring-blue-300"></td>`;
            }
            if (col.type === 'dropdown') {
                const optionsHtml = col.options ? col.options.map(opt => `<option value="${opt}">${opt}</option>`).join('') : '';
                return `<td class="p-1 border border-black bg-orange-50"><select id="${fieldId}" name="${fieldId}" onchange="calculateRow(${s.id})" class="w-full h-full p-1 text-center bg-transparent outline-none text-sm cursor-pointer"><option value="">-</option>${optionsHtml}</select></td>`;
            }
            if (col.type === 'formula') {
                return `<td class="p-1 border border-black bg-yellow-50"><input type="text" id="${fieldId}" name="${fieldId}" readonly tabindex="-1" class="w-full h-full p-1.5 text-center bg-transparent outline-none font-bold text-gray-700" value="-"></td>`;
            }
        ...
        <tr class="hover:bg-gray-50">
            ...
            <td class="p-2 bg-white text-sm text-gray-800 font-bold whitespace-nowrap border border-black sticky left-10 z-10 shadow-[2px_0_5px_rgba(0,0,0,0.05)]">${s.namaLengkap}</td>
